...
const BalanceBoard: React.FC<BalanceProps> = ({ usdBalance }) => {
  ...
  return (
    <div className='bg-[#4B50661A] rounded-[18px] border-[0.5px] border-[#222326] w-[329px] h-[130px] flex flex-col items-center justify-center mx-auto'>
      ...
      <p className='text-[#fff] flex gap-[7px] justify-center font-bold text-[16px]'>${truncateWithoutRounding(Number(stats?.usd),2)} <span className='justify-center font-bold flex items-center bg-[#0BA0244D] text-[14px] rounded-[41px] text-[#54D56A] w-[70px] h-[25px] text-center'>
        {stats?.precentage?.includes('-') ? '-' : '+'} {truncateWithoutRounding(Number(stats?.precentage),2)}%
      </span>
      </p>
    </div>
  );
